(ns smee.theta
  (:require [smee.middleware :as middleware]
            [smee.router :as router]
            [smee.dev.server :as dev.server]
            [smee.prod.server :as prod.server]
            [smee.env :as env]
            [smee.components]))
...
(defn app
  "The main entry point for smee apps"
  [opts]
...
(defn server
  "Runs an http-kit server based on the SMEE_ENV env variable, options are dev, test or prod"
  ([app]
   (server app nil))
  ([app opts]
   (if (= "dev" (env/env :smee-env))
...
(defn url-for
  ...
  (This function requires `smee/app` to be invoked first.)"
  ([k m]
   (if-let [anchor (:# m)]
...
(defn form-for
  ...
  (let [m    (if (map? (first body))
        ...
        body (if (map? (first body))
               (drop 1 body)
               body)]
    (smee.components/form (merge (action-for k m) opts)
                           body)))
