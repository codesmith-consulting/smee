...
(defn insert
  ...
  [val]
  (when (nil? val)
    (throw (Exception. "smee/insert was called with nil")))
  (let [v (utils/vectorize val)
        ...
        _ (when (not (empty? unqualified-ks))
            (throw (Exception. "smee/insert requires all maps to have qualified keys. The namespace is used to determine the table to insert into.")))
        distinct-namespaces (distinct
                             (map namespace cols))
        _ (when (not= 1 (count distinct-namespaces))
            (throw (Exception. (str "smee/insert found multiple namespaces. "
...
(defn update
  ...
  (let [v (utils/vectorize val)
        ...
        _ (when (nil? table)
            (throw (Exception. "smee/update requires all maps to have qualified keys, could not find table to update")))
        pk (first (filter #(= "id" (name %)) (-> v first keys)))
        pk-values (mapv #(get % pk) v)]
    (if (not= 0 (count
                 (filter nil? pk-values)))
      (throw (Exception. (str "smee/update found no value for the :id key in the following maps: " (vec (filter #(nil? (get % pk)) v)))))
      (concat
        [:update table
         :where [pk pk-values]]
...
(defn upsert
  ...
  [val opts]
  (when (nil? val)
    (throw (Exception. "smee/upsert was called with nil")))
  (let [v (utils/vectorize val)
        cols (->> (mapcat identity v)
                  (map first)
                  (distinct))
        unqualified-ks (filter #(not (qualified-ident? %)) cols)
        _ (when (not (empty? unqualified-ks))
            (throw (Exception. "smee/upsert requires all maps to have qualified keys. The namespace is used to determine the table to insert into.")))
        distinct-namespaces (distinct
                             (map namespace cols))
        _ (when (not= 1 (count distinct-namespaces))
            (throw (Exception. (str "smee/upsert found multiple namespaces. "
                                    "Which table did you want to insert into?"
                                    (string/join ", " distinct-namespaces)))))
        values (map #(map (fn [x] (second x)) %) v)
...
(defn delete
  ...
  (let [v (utils/vectorize val)
        table (-> v first keys first namespace)
        _ (when (nil? table)
            (throw (Exception. "smee/delete requires all maps to have qualified keys, could not find table to delete from")))
        pk (first (filter #(= "id" (name %)) (-> v first keys)))
        _ (when (nil? pk)
            (throw (Exception. "smee/delete couldn't find the qualified :id key")))]
    [:delete
     :from table
