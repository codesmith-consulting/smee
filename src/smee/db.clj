(ns smee.db
  (:require [clojure.java.jdbc :as jdbc]
            ...
            [smee.db.queries :as queries]
            [smee.db.transact :as db.transact]
            [smee.db.connection :as db.connection :refer [connection spec]]
            [smee.db.update :as db.update]
            [smee.db.insert :as db.insert]
            [smee.db.schema :as db.schema]
            [smee.db.sql :as sql]
            [smee.db.helpers :as helpers]
            [smee.migrations :as migrations]
            [smee.utils :as utils]
            [smee.error :refer [raise rescue]]
...
(defn first!
  ...
  [coll]
  (or (first coll)
      (raise "Record not found" {:smee.router/error :404
                                 :404                true
                                 :not-found          true
                                 :type               :404
                                 ::error             :not-found})))
...
(defn pluck
  ...
  ```clojure
  (smee/pluck [:select * :from person :where [id 1]])
  ```
  "
  ([conn v params]
   (first
...
(defn insert
  ...
  ```clojure
  (smee/insert {:person/email \"test@example.com\" :person/screen-name \"test\"})
  ```

  You can also insert multiple records at once:

  ```clojure
  (smee/insert [{:person/email \"test1@test.com\" :person/screen-name \"test1\"}
...
(defn update
  ...
  ```clojure
  (smee/update {:person/id 1 :person/last-name \"Appleseed\" :person/first-name \"Johnny\"})
  ```

  It can also take a list of maps:

  ```clojure
  (smee/update [{:person/id 1 :person/last-name \"Appleseed\"}
...
(defn upsert
  ...
  ([conn arg opts]
   (let [table-name        (if (sequential? arg)
     ...
     (when (nil? table-name)
       (throw (Exception. "smee/upsert expects a map with qualified keywords")))
