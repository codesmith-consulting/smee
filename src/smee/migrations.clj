(ns smee.migrations
  (:require [smee.migrations.sql :as migrations.sql]
            [smee.migrations.edn :as migrations.edn]
            [smee.db.migrations]
            [smee.db.connection :refer [connection]]
            [smee.db.schema :as schema]
...
(defn create-table []
  (jdbc/db-do-commands (connection)
    (jdbc/create-table-ddl :smee_schema_migrations
...
(defn completed-migrations []
  ...
  (->> (jdbc/query
        (connection)
        ["select version from smee_schema_migrations order by version"]
        {:keywordize? true})
       (map :version)))
...
(defn migrate []
  (let [migrations (pending)]
    (reset! smee.db.migrations/rollback? false)
      ...
      (let [statements (statements migration)
        ...
        (if (string/blank? (:sql statements))
          ...
          (do
            ...
            (jdbc/insert! (connection) :smee_schema_migrations {:version (version migration)})
  ...
  (let [migration-type (last
    ...
    (condp = migration-type
      ...
      (let [f (load-file filename-with-path)
            ...
            output (cond
                     down   (do
                              (reset! smee.db.migrations/rollback? false) ;; don't auto-undo anything,
                              (down)             ;; because we're gonna run 'down' exactly as provided;
                              (reset! smee.db.migrations/rollback? true)) ;; and then put it back.
                     change (change)
                     :else  nil)]
        {:sql (string/join "" output)
         :vec output
...
(defn rollback []
  (let [migration (migration-filename (last (completed-migrations)))
        _ (reset! smee.db.migrations/rollback? true)]
    (when (some? migration)
        (let [statements (rollback-statements migration)
          ...
          (if (string/blank? (:sql statements))
            ...
            (do
              ...
              (println "")
              (jdbc/db-do-commands (connection) (or (:vec statements) (:sql statements)))
              (jdbc/delete! (connection) :smee_schema_migrations ["version = ?" (version migration)])
              (when (.endsWith migration ".edn")
                (schema/save (:raw statements)))
