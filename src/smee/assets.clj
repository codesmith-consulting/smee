...
(defn bundle [smee-env bundle-name]
  (let [s (if (= "prod" smee-env) "assets.minified.edn" "assets.edn")
        res (io/resource s)]
    (if (some? res)
      (let [m (-> res slurp edn/read-string)]
        (if (= "prod" smee-env)
          ...
          (get (->> (map (fn [[k v]] {k (hrefs (ext k) v)}) m)
                    (apply merge))
               bundle-name)))
      (println "Warning: You're trying to load" s "but it doesn't exist! Run `SMEE_ENV=prod make assets` for minified, bundled assets in production"))))
